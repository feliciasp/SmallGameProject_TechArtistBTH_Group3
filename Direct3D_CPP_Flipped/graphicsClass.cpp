...
bool graphicsClass::frame(objectClass* object, XMMATRIX view, XMMATRIX proj, int type, std::string name, XMVECTOR camPos, int frameCount, int currentFrame, bool flipped)
{
	bool result;
	//render graphics scene
	result = render(object, view, proj, type, name, camPos, frameCount, currentFrame, flipped);
	if (!result)
	{
...
bool graphicsClass::render(objectClass* object, XMMATRIX view, XMMATRIX proj, int type, std::string name, XMVECTOR camPos, int frameCount, int currentFrame, bool flipped)
{
	HRESULT result;
	XMMATRIX tempWorld;
	...
	for (int i = 0; i < object->getMeshCount(); i++)
	{
		...
		result = shaders->render(direct3D->GetDeviceContect(), object->getVertexCount(i), tempWorld, view, proj, type, name, camPos, frameCount, currentFrame, flipped);
		if (!result)
		{
