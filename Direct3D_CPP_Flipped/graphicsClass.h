...
class graphicsClass
{
	public:
		...
		void shutdown();
		bool frame(objectClass* object, XMMATRIX view, XMMATRIX proj, int type, std::string name, std::string name2, XMVECTOR camPos, XMMATRIX joints[30], int weaponType, int hurt = 0, int frameCount = 1, int currentFrame = 1, int currentAnimation = 1, bool flipped = false);
		void endScene();
		void beginScene();
